    ...
    <xsl:template name="input">
        ...
        <xsl:choose>
            <xsl:when test="$type='xsd-boolean'">
                <input id="{$id}-value" type="checkbox" name="{$name}" class="xfValue">
                    ...
                    <xsl:if test="exists(xf:hint)">
                        <xsl:attribute name="title" select="xf:hint/text()"/>
                    </xsl:if>
                </input>
            </xsl:when>
            <xsl:otherwise>
...
-->
    ...
    <xsl:template name="output">
        ...
        <xsl:choose>
            ...
            <xsl:when test="string-length(../xf:label) != 0">
    ...
    <xsl:template name="drawRangeBasic">
        ...
        <xsl:if test="$current &lt;= $end">
            <option id="{$rangeId}-value" value="{$current}" title="{xf:hint}" class="xfSelectorItem">
...
<!-- create the  selectionlist that is shown to the user: the CLONE -->
    ...
    <xsl:template name="upload">
        ...
        <xsl:element name="input">
            ...
            <xsl:attribute name="class">xfValue</xsl:attribute>
			<!-- Content types accepted, from mediatype xf:upload attribute
            to accept input attribute -->
            <!--
                        <xsl:attribute name="accept">
                            <xsl:value-of select="translate(normalize-space(@xf:mediatype),' ',',')"/>
                        </xsl:attribute>
            -->
            <xsl:if test="exists(xf:hint)">
                <xsl:attribute name="title" select="xf:hint/text()"/>
	...
	<xsl:template name="build-items-item">
		<option id="{@id}-value" value="{xf:value}" title="{xf:hint}" class="xfSelectorItem">
	...
	<xsl:template name="build-checkboxes-item">
        ...
        <span id="{@id}" class="xfSelectorItem">
            <input id="{@id}-value" class="xfValue" type="checkbox" name="{$name}">
                <xsl:if test="string-length($navindex) != 0">
                    <xsl:attribute name="tabindex">
                        <xsl:value-of select="$navindex"/>
                    </xsl:attribute>
	...
	<xsl:template name="build-radiobuttons-item">
        ...
        <xsl:param name="navindex"/>         
        <span id="{@id}" class="xfSelectorItem">
            <input id="{@id}-value" class="xfValue" type="radio" name="{$name}">
                <xsl:if test="string-length($navindex) != 0">
                    <xsl:attribute name="tabindex">
                        <xsl:value-of select="$navindex"/>
                    </xsl:attribute>
