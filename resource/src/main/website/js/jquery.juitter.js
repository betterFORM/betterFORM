					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<a class='twitterUsername' href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /></a> "+$.Juitter.textFormat(tweet)+" - <span class='time'>"+item.created_at.substring(0,item.created_at.length - 15)+"</span> <a class='twitterReadMore' href='" + link + "' class='JRM' "+openLink+">"+readMore+"</a>";
								else mHTML="<a class='twitterUsername' href='http://www.twitter.com/"+item.from_user+"'>@"+item.from_user+":</a> "+$.Juitter.textFormat(tweet)+" - <span class='time'>"+item.created_at.substring(0,item.created_at.length - 15)+"</span> <a class='twitterReadMore' href='" + link + "' "+openLink+">"+readMore+"</a>";
