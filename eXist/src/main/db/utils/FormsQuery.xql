		...
		<div dojoType="dijit.TooltipDialog" name="collectionTooltip" >
			<form enctype="text/plain" method="get" action="{$xqeuryPath}create-collection.xql" target="targetFrame">
					<fieldset>
						...
						<input type="path" name="path" value="{$path}" style="display:none"/>
...
(: ORIGINAL
declare function local:generateUploadMarkup($uri as xs:string, $contextPath as xs:string, $altTextFormUpload as xs:string) {
	let $xqeuryPath := functx:substring-before-last($uri, 'FormsQuery.xql')
	return
	...
	if ( fn:not('betterform' eq $path) )
		then(
			<div id="bfListView">
				...
				{local:generateDirectory($uri, $contextPath, $path, $ajaxFunction, $childCollection)}
			</div>
		) else (
			local:generateDirectory($uri, $contextPath, $path, $ajaxFunction, $childCollection)
		)
