 ...
 <xsl:template match="/">
     ...
     <library>
         <xsl:for-each-group select="$iTunesLibTransformationStep1//track" group-by="album">			
             <album>
                 <xsl:element name="artist"><xsl:value-of select="normalize-space(current-group()[1]//artist[1]/text())"></xsl:value-of></xsl:element>
                 <xsl:element name="name"><xsl:value-of select="normalize-space(current-grouping-key())"/></xsl:element>
                 <xsl:element name="genre"><xsl:value-of select="normalize-space(current-group()[1]//genre[1]/text())"/></xsl:element>
                 
                 <xsl:for-each select="current-group()">
                     <track>
                         <xsl:copy-of select="@id"/>
                         <xsl:copy-of select="@trackNumber"/>
                         <xsl:copy-of select="@trackCount"/>
    ...
    <xsl:template name="track">
        <xsl:element name="track">
            ...
            <xsl:attribute name="album" select="child::*[preceding-sibling::* = 'Album'][1]"/>
           <!--  
            ...
            -->
            <name><xsl:value-of select="child::*[preceding-sibling::* = 'Name'][1]" /></name>
            <artist><xsl:value-of select="child::*[preceding-sibling::* = 'Artist'][1]" /></artist>
            <album><xsl:value-of select="child::*[preceding-sibling::* = 'Album'][1]" /></album>
            <genre><xsl:value-of select="child::*[preceding-sibling::* = 'Genre'][1]" /></genre>
            <kind><xsl:value-of select="child::*[preceding-sibling::* = 'Kind'][1]" /></kind>
