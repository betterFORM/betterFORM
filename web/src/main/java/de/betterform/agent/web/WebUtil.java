...
import de.betterform.agent.web.rest.RestException;
...
/**
 * Collection of static methods to be re-used by different Servlets/Filter
 * implementations.
 * 
 * @author Joern Turner
 */
public class WebUtil {
		...
		if (LOGGER.isDebugEnabled()) {
			...
			if (keys.hasMoreElements()) {
				...
				while (keys.hasMoreElements()) {
					...
					LOGGER.debug("existing sessionkey: " + s + ":"
	 ...
	 */
	public static WebProcessor getWebProcessor(HttpSession session) {
		...
		// return null;
		// } else {
		return getWebProcessor(session, "webprocessor");
		// }
	}
	...
	public static WebProcessor getWebProcessor(HttpServletRequest request) {
		...
		// session.getAttribute(XFormsSessionManager.XFORMS_SESSION_MANAGER);
		// XFormsSession xFormsSession = manager.getWebProcessor(key);
		return getWebProcessor(request, key);
	}

	public static WebProcessor getWebProcessor(HttpSession session, String key) {
		//Cache cache = CacheManager.getInstance().getCache("xfSessionCache");
		if (session == null || session.getAttribute(key) == null) {
			LOGGER.warn("No xformsSession for key " + key + " in Session");
			return null;
		}
		
		return (WebProcessor) session.getAttribute(key);
		
	}
	
	public static WebProcessor getWebProcessor(HttpServletRequest request,String key) {
		...
		return getWebProcessor(request.getSession(), key);
	...
	public static boolean removeSession(HttpServletRequest request) {
		...
		return removeSession(request, key);
	}

	public static boolean removeSession(HttpSession session, String key) {
		
		WebProcessor processor = getWebProcessor(session, key);
	...
	public static boolean removeSession(HttpServletRequest request, String key) {

		if (key == null || key.equals("")) {
			LOGGER.warn("SessionKey is null");
			return false;
		}
		return removeSession(request.getSession(), key);
	 ...
	 * <p/>
	 * NOTE: this method should be called *before* the Adapter is initialized
	 * cause the cookies may already be needed for setup (e.g. loading of XForms
	 * via Http)
	 */
	public static void storeCookies(HttpServletRequest request,
		...
		if (cookiesIn != null) {
			...
			for (int i = 0; i < cookiesIn.length; i += 1) {
				...
				commonsCookies[i] = new BasicClientCookie(c.getName(),
						c.getValue());
				commonsCookies[i].setDomain(c.getDomain());
				commonsCookies[i].setPath(c.getPath());
				commonsCookies[i].setAttribute(ClientCookie.MAX_AGE_ATTR,
	...
	/**
	 * copy all http headers from client request into betterForm context map as
	 * map HTTP_HEADERS. This map will be picked up in AbstractHttpConnector to
	 * configure a proper request for the internal http-client.
	 */
	public static void copyHttpHeaders(HttpServletRequest request,
			XFormsProcessor processor) {
		...
		while (headerNames.hasMoreElements()) {
			...
			if (WebUtil.LOGGER.isDebugEnabled()) {
				WebUtil.LOGGER.debug("keeping httpheader: " + headerName
						+ " value: " + request.getHeader(headerName));
			}

		}
		processor.setContextParam(AbstractHTTPConnector.HTTP_REQUEST_HEADERS,
				httpHeaders);
	}

	/**
	 * returns an URL which is passed as request param and decodes it if
	 * necessary
	 * 
	 * @param request
	...
	/**
	 * this method is responsible for passing all context information needed by
	 * the Adapter and Processor from ServletRequest to Context. Will be called
	 * only once when the form-session is inited (GET).
	 * <p/>
	 * <p/>
	 * todo: better logging of context params
	 * 
	 * @param request
	 *            the Servlet request to fetch params from
	 * @param httpSession
	 *            the Http Session context
	 * @param processor
	 *            the XFormsProcessor which receives the context params
	 * @param sessionkey
	 *            the key to identify the XFormsSession
	 */
	public static void setContextParams(HttpServletRequest request,
			HttpSession httpSession, XFormsProcessor processor,
			String sessionkey) {
		Map servletMap = new HashMap();
		servletMap.put(WebProcessor.SESSION_ID, sessionkey);
		processor.setContextParam(XFormsProcessor.SUBMISSION_RESPONSE,
				servletMap);

		// adding requestURI to context
		processor.setContextParam(WebProcessor.REQUEST_URI,
				WebUtil.getRequestURI(request));
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("context root of webapp: "
					+ processor.getContextParam(WebProcessor.CONTEXTROOT));
		}
		...
		if (requestPath.length() != 0) {
			// adding request path e.g.
			// '/betterform/forms/demo/registration.xhtml'
			processor.setContextParam(WebProcessor.REQUEST_PATH, requestPath);

			// adding filename of requested doc to context
			String fileName = requestPath.substring(
					requestPath.lastIndexOf('/') + 1, requestPath.length());// FILENAME
																			// xforms
			processor.setContextParam(FILENAME, fileName);

			if (requestURL.contains(contextRoot)) { // case1: contextRoot is a
													// part of the URL
				// adding plainPath which is the part between contextroot and
				// filename e.g. '/forms' for a requestPath of
				// '/betterform/forms/Status.xhtml'
			...
			} else {// case2: contextRoot is not a part of the URL take the part
					// previous the filename.
				String[] urlParts = requestURL.split("/");
				plainPath = urlParts[urlParts.length - 2];
			}

			// adding contextPath - requestPath without the filename
			processor.setContextParam(CONTEXT_PATH, contextRoot + "/"
					+ plainPath);
		}
		...
		// adding pathInfo to context - attention: this is only available when a
		// servlet is requested
		String s1 = request.getPathInfo();
		if (s1 != null) {
			processor.setContextParam(WebProcessor.PATH_INFO, s1);
		}
		processor.setContextParam(WebProcessor.QUERY_STRING, (request
				.getQueryString() != null ? request.getQueryString() : ""));
		...
		processor.setContextParam(
				TransformerService.TRANSFORMER_SERVICE,
				httpSession.getServletContext().getAttribute(
						TransformerService.TRANSFORMER_SERVICE));
		if (LOGGER.isDebugEnabled()) {
		...
		// [2] read any request params that are *not* betterForm params and pass
		// them into the context map
		Enumeration params = request.getParameterNames();
		String s;
		while (params.hasMoreElements()) {
			s = (String) params.nextElement();
			// store all request-params we don't use in the context map of
			// XFormsProcessorImpl
	...
	public static boolean isMediaTypeXML(String s) {
		...
		isXML = s.endsWith("+xml")
				|| ((s.equals("text") || s.equals("application")) && (s
						.equals("xml") || s
						.equals("xml-external-parsed-entity")));
	 ...
	 * @param input
	 *            an DOM input document to transform
	 * @param stylesheetName
	 *            the name of the stylesheet to use. This must be preloaded in
	 *            CachingTransformerService. See WebFactory
	 * @param params
	 *            transformation parameters as a piece of DOM if any. The params
	 *            object is passed as param 'params' to the stylesheets
	 * @throws java.io.IOException
	 */
	public static void doTransform(ServletContext context,
			HttpServletResponse response, Document input,
		...
		try {
			Transformer transformer = transformerService
					.getTransformerByName(stylesheetName);
			if (params != null) {
