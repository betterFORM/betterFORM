...
declare function local:main() as node()?
{
	...
	let $durationHours     := if(sum($selectedTasks//duration/@hours) > 0)
	...
	let $durationMinutes   := if(sum($selectedTasks//duration/@minutes) > 0)
	                          then concat(substring-before(xs:string(sum($selectedTasks//duration/@minutes) div 60),'.'),':', (sum($selectedTasks//duration/@minutes)) mod 60)
	                          else '0:0'
	let $duration          := if($minutesRest > 0)
	                          then concat($durationHours + xs:double(substring-before($durationMinutes,':')),':',substring-after($durationMinutes,':'))
	                          else concat($durationHours+$sumMinutesInHours,':0')
